...
program
    ...
    .action(async () => {
    try {
        await (0, login_1.loginCommand)(API_URL);
    }
    catch (err) {
...
program
    ...
    await (0, scan_1.scanCommand)(targetPath, options, API_URL);
});
program
    .command("whoami")
    ...
    const user = await (0, auth_1.validateTokenWithServer)(API_URL, token);
    if (user) {
