    ...
    for (const file of files) {
        ...
        lines.forEach((line, index) => {
            if (/(api[_-]?key|secret|token|password)\s*[:=]\s*["'][^"']{8,}/i.test(line))
                issues.push({ file: file.path, line: index + 1, severity: "critical", category: "security", title: "Potential hardcoded secret", description: "A credential-like value appears inline in source code.", fix: "Move this value to an environment variable or secret manager." });
            if (/select .+ \+|query\(.+\+/.test(line))
                issues.push({ file: file.path, line: index + 1, severity: "critical", category: "security", title: "Possible SQL injection", description: "A query appears to concatenate values directly into SQL.", fix: "Use parameterized queries instead of string concatenation." });
            if (/catch\s*\([^)]*\)\s*\{\s*\}/.test(line))
                issues.push({ file: file.path, line: index + 1, severity: "warning", category: "error handling", title: "Empty catch block", description: "Errors are swallowed without logging or recovery.", fix: "Log the error or return a typed failure path." });
        });
    }
